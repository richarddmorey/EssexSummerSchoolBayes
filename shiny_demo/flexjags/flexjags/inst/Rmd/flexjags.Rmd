...
do_sampling = eventReactive(input$do_sample,{
  ...
  shiny::withProgress(message = "Sampling...", value = 0, {
    ...
    if(engine == "JAGS"){
      ...
      samples = jags_samples(pack, model, data_html, post_txt, result, n_iter, n_progress, n_chain, cur_trace, eliminate_data, render, session)
      
    }else if(engine == "stan"){
      
      samples = stan_samples(pack, model, data_html, post_txt, result, n_iter, n_progress, n_chain, cur_trace, eliminate_data, render, session)
